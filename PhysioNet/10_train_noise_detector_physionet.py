# -*- coding: utf-8 -*-
"""
PhysioNet 噪声判别器训练（EfficientNet-B0，两个变体对比）

与 CQ500 的 10 同构。变体共用 09 生成的同一份 7 类数据集，只是类别子集不同：
  6class —— clean + 5 种单噪声，共 6 类。复合噪声对它完全**分布外**：
            只能把复合噪声硬分到某个已知单噪声类，永远无法路由到"复合噪声最优 filter"。
  7class —— 额外把复合未知噪声作为第 7 类显式学习，可直接识别并路由。

判别器按患者划分 train/val：沿用 09 manifest 的 source_split
（PhysioNet train 患者训练、val 患者验证），两侧都不含 test 患者。

**必须是 PhysioNet 自训练的判别器**：旧版直接复用 CQ500 的权重，
那是跨域使用（不同预处理、不同层厚、不同机器），不成立。

训练后一律再喂全部 7 种条件做逐类评估：对 6class 故意暴露其在复合噪声上的行为。

权重: models/noise_detector_efficientnetb0_physionet_{variant}_best.pth
输出: results/noise_detector_physionet/{variant}/

用法:
    python 10_train_noise_detector_physionet.py --variant 6class
    python 10_train_noise_detector_physionet.py --variant 7class
"""
import argparse
import sys
...
from physionet_commons import (DETECTOR_CLASSES, DETECTOR_DATASET_DIR, DETECTOR_VARIANTS,
...
class DetectorDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # get(..., -1)：评估分布外条件（如 6class 遇到 unknown_mixed）时标签无意义
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=0, help="每类仅取前 N 张（冒烟测试）")
    ...
    if not manifest.exists():
        raise FileNotFoundError(f"缺少 {manifest}，请先运行 09_build_noise_detector_dataset_physionet.py")
    ...
    assert not (tr_pat & va_pat), f"判别器 train/val 患者重叠: {sorted(tr_pat & va_pat)[:5]}"
    ...
    for epoch in range(args.epochs):
        ...
        print(f"[{variant}] epoch {epoch+1}/{args.epochs}: loss={total_loss/max(n,1):.4f} "
    ...
    plt.title(f"Noise Detector PhysioNet [{variant}] — val acc={best_acc:.4f}",
    ...
    for cond in DETECTOR_CLASSES:
        ...
        print(f"  {cond:15s} {'在类内' if in_vocab else '分布外(无该类)'}  acc={shown} "
