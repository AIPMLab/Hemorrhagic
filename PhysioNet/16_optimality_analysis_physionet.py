# -*- coding: utf-8 -*-
"""
PhysioNet 复合未知噪声：Q1 恢复性 + Q2 最优性分析（与 CQ500 的 16 同构）
...
输出: results/optimality_physionet/{variant}/

用法:
    python 16_optimality_analysis_physionet.py --variant 7class
"""
import argparse
import sys
...
from physionet_commons import (CLASSES, DATA_ROOT, DETECTOR_CLASSES, DETECTOR_VARIANTS,
...
ADAPTIVE_CSV = (RESULT_ROOT / "adaptive_unknown_physionet" / "{variant}"
                / "adaptive_unknown_results_physionet.csv")
SEL_CSV = (RESULT_ROOT / "adaptive_unknown_physionet" / "{variant}"
           / "unknown_filter_selection_log_physionet.csv")


def variant_outdir(variant):
    return RESULT_ROOT / "optimality_physionet" / variant
...
# mcnemar / boot_ci 已移入共享层 ich_common，19 / 20 也复用同一实现，避免两处口径漂移
...
def main():
    ...
    oracle_df.to_csv(OUTDIR / "oracle_summary_physionet.csv", index=False)
    pd.DataFrame(sweep_rows).to_csv(OUTDIR / "filter_sweep_unknown_mixed_physionet.csv", index=False)
    pp_df = pd.DataFrame(pp_rows)
    pp_df.insert(0, "variant", variant)
    pp_df.to_csv(OUTDIR / "per_patient_oracle_physionet.csv", index=False)
    ...
    if not args.skip_detector and detector_weight_path(variant).exists():
        ...
        det_df.to_csv(OUTDIR / "detector_accuracy_physionet.csv", index=False)
    ...
    with open(OUTDIR / "latex_q1_q2_summary.tex", "w", encoding="utf-8") as fh:
        fh.write(oracle_df.to_latex(index=False, float_format="%.4f", escape=False,
                                    caption="PhysioNet ICH subset: Q1 recovery and Q2 optimality "
                                            "of the adaptive pipeline on compound unknown noise "
                                            "(patient-level).",
                                    label="tab:physionet_q1q2"))
