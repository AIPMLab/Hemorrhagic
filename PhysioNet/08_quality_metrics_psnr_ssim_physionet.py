# -*- coding: utf-8 -*-
"""
PhysioNet 图像质量指标：原始 test vs 各增强方法输出的 PSNR/SSIM

路径修正：增强图在 enhanced_tests/physionet/{noise}/{method}/...，
...
输出: results/quality_metrics_physionet/quality_summary_physionet.csv, all_quality_metrics_physionet.csv

用法:
    python 08_quality_metrics_psnr_ssim_physionet.py
    python 08_quality_metrics_psnr_ssim_physionet.py --limit 200
"""
import argparse
import sys
...
from physionet_commons import (DATA_ROOT, FILTER_NAMES, NOISE_NAMES, NOISY_ROOT,
...
def main():
    ...
    out = RESULT_ROOT / "quality_metrics_physionet"
    out.mkdir(parents=True, exist_ok=True)
    all_df.to_csv(out / "all_quality_metrics_physionet.csv", index=False)
    ...
    summary.to_csv(out / "quality_summary_physionet.csv", index=False)
