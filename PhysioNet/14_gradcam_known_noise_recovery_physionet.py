# -*- coding: utf-8 -*-
"""
PhysioNet 已知噪声最优增强恢复 Grad-CAM（与 CQ500 的 14 同构）

对 5 类已知噪声，用 07 输出的 best_filter_map_physionet.csv 选出的最优方法，
对比 Raw / Noisy / Best-Enhanced 三种输入的 Grad-CAM。

注意增强图路径含**噪声层级**：enhanced_tests/physionet/{noise}/{method}/...
（enhanced_dir 是唯一入口；旧写法漏掉噪声层会取不到图）

输出: results/gradcam_known_noise_recovery_physionet/{noise}/{patient}/*.png

用法:
    python 14_gradcam_known_noise_recovery_physionet.py
    python 14_gradcam_known_noise_recovery_physionet.py --model swin_tiny
"""
import argparse
import sys
...
from physionet_commons import (DATA_ROOT, MODEL_DIR, NOISY_ROOT, RESULT_ROOT, IMG_SIZE,
...
OUT_DIR = RESULT_ROOT / "gradcam_known_noise_recovery_physionet"
ARCH = {"resnet50": "resnet50",
        "swin_tiny": "swin_tiny_patch4_window7_224"}[args.model]
WEIGHT = MODEL_DIR / f"{ARCH}_physionet_best.pth"
BEST_CSV = RESULT_ROOT / "enhanced_known_noise_physionet" / "best_filter_map_physionet.csv"
