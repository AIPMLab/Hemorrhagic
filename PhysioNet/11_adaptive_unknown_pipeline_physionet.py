# -*- coding: utf-8 -*-
"""
PhysioNet 自适应未知噪声管线（per-slice），支持两个判别器变体

与 CQ500 的 11 同构：
  1) 指定变体的**PhysioNet 自训练**判别器对 unknown_mixed 每张测试切片预测噪声类别
...
输出（按变体分目录）:
  results/adaptive_unknown_physionet/{variant}/adaptive_unknown_results_physionet.csv
  results/adaptive_unknown_physionet/{variant}/unknown_noise_detection_log_physionet.csv
  results/adaptive_unknown_physionet/{variant}/unknown_filter_selection_log_physionet.csv

用法:
    python 11_adaptive_unknown_pipeline_physionet.py --variant 6class
    python 11_adaptive_unknown_pipeline_physionet.py --variant 7class
"""
import argparse
import sys
...
from physionet_commons import (ADAPTIVE_ROOT, CLASSES, DATA_ROOT, DETECTOR_VARIANTS,
...
MAP_CSV = RESULT_ROOT / "enhanced_known_noise_physionet" / "best_filter_map_physionet.csv"


def variant_outdir(variant):
    return RESULT_ROOT / "adaptive_unknown_physionet" / variant
...
def load_filter_map():
    if not MAP_CSV.exists():
        raise FileNotFoundError(f"缺少 {MAP_CSV}，请先运行 "
                                f"07_evaluate_enhanced_known_noise_physionet.py")
...
def main():
    ...
    for mname, (arch, wpath) in models.items():
        if not wpath.exists():
            raise FileNotFoundError(f"缺少分类器权重 {wpath}，请先运行 02_train_physionet.py")
    ...
    det_log.to_csv(OUTDIR / "unknown_noise_detection_log_physionet.csv", index=False)
    pd.DataFrame(sel_rows).to_csv(
        OUTDIR / "unknown_filter_selection_log_physionet.csv", index=False)

    summary = pd.DataFrame(rows).sort_values(["model", "condition"])
    summary.to_csv(OUTDIR / "adaptive_unknown_results_physionet.csv", index=False)
