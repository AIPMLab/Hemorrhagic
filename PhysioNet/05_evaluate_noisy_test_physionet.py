# -*- coding: utf-8 -*-
"""
PhysioNet 加噪图患者级评测：每个噪声类型 -> 患者级指标

输出: results/noisy_evaluation_physionet/{model}/... 与 all_noisy_results_physionet.csv

用法:
    python 05_evaluate_noisy_test_physionet.py
    python 05_evaluate_noisy_test_physionet.py --models resnet50 --limit 300
"""
import argparse
import sys
...
from physionet_commons import (MODEL_DIR, NOISY_ROOT, NOISE_NAMES, RESULT_ROOT,
...
def main():
    ...
    outdir = RESULT_ROOT / "noisy_evaluation_physionet"
    ...
    for mname, (arch, wpath) in models.items():
        ...
        for noise in noises:
            ...
            if not base.exists():
                print(f"[skip] 缺少 {base}，请先运行 04_generate_noisy_test_physionet.py")
                continue
    ...
    summary.to_csv(outdir / "all_noisy_results_physionet.csv", index=False)
    print("\n[summary]")
