# -*- coding: utf-8 -*-
"""
PhysioNet 可视化：已知噪声增强性能 热力图 + 分面柱状图（患者级指标）

读 07 产出的 all_enhanced_known_noise_physionet.csv（该表已按 val 拟合 / test 评估口径生成）。
输出: results/enhanced_known_noise_physionet/fig_heatmap_physionet_{metric}.png, fig_bar_physionet.png
"""
import sys
...
DATA = RESULT_ROOT / "enhanced_known_noise_physionet" / "all_enhanced_known_noise_physionet.csv"
...
def main():
    if not DATA.exists():
        raise SystemExit(f"缺少 {DATA}，请先运行 07_evaluate_enhanced_known_noise_physionet.py")
    ...
    for metric in METRICS:
        ...
        fig.suptitle("PhysioNet Known-Noise Enhancement (Patient-level)", y=1.02,
                     fontweight="bold")
        fig.savefig(OUTDIR / f"fig_heatmap_physionet_{metric}.png", dpi=150, bbox_inches="tight")
        plt.close()
    ...
    fig.suptitle("PhysioNet Enhancement Accuracy by Noise (Patient-level)", y=1.02,
                 fontweight="bold")
    fig.savefig(OUTDIR / "fig_bar_physionet.png", dpi=150, bbox_inches="tight")
    plt.close()
    print("[done] bar")
