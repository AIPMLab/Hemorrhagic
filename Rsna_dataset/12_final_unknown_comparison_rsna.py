# -*- coding: utf-8 -*-
"""
RSNA 最终对比：Raw / Unknown Noisy / Adaptive Enhanced 患者级指标汇总表与图
（数据来自 11 --variant X 的输出）

输出: results/final_unknown_comparison_rsna/{variant}/

用法:
    python 12_final_unknown_comparison_rsna.py --variant 7class
"""
import argparse
import sys
...
in_file = (RESULT_ROOT / "adaptive_unknown_rsna" / args.variant
           / "adaptive_unknown_results_rsna.csv")
out = RESULT_ROOT / "final_unknown_comparison_rsna" / args.variant
...
summary.to_csv(out / "final_unknown_comparison_summary_rsna.csv", index=False)
...
ax.set_title(f"RSNA ICH [{args.variant}]", fontweight="bold")
plt.tight_layout()
plt.savefig(out / "fig_patient_accuracy_rsna.png", dpi=150)
plt.close()
print(f"\n[done] 图已保存: {out / 'fig_patient_accuracy_rsna.png'}")
