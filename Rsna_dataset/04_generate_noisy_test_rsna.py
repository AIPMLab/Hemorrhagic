# -*- coding: utf-8 -*-
"""
RSNA 噪声测试集生成：对 test 患者切片施加 6 类噪声

噪声实现全部来自 rsna_commons（即 cq500_commons 那一份）——
与 CQ500 完全同一批函数，避免两套数据集"噪声参数不同"导致对比失效，
也避免判别器训练数据与测试数据参数不一致。

输出: noisy_tests/rsna/{noise}/{label}/{patient_id}/sXX_fXXXX.png

用法:
    python 04_generate_noisy_test_rsna.py
    python 04_generate_noisy_test_rsna.py --limit 200      # 冒烟测试
"""
import argparse
import sys
...
from rsna_commons import (DATA_ROOT, NOISE_NAMES, NOISY_ROOT, apply_noise,
...
def main():
    ...
    pd.DataFrame({"noise": NOISE_NAMES}).to_csv(
        NOISY_ROOT.parent / "rsna_noise_list.csv", index=False)
