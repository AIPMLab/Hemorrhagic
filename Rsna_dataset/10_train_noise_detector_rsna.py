# -*- coding: utf-8 -*-
"""
RSNA 噪声判别器训练（EfficientNet-B0，两个变体对比）
...
（RSNA train 患者训练、val 患者验证），两侧都不含 test 患者。

**必须是 RSNA 自训练的判别器**：旧版直接复用 CQ500 的权重，
那是跨域使用（不同预处理、不同层厚、不同机器），不成立。

训练后一律再喂全部 7 种条件做逐类评估：对 6class 故意暴露其在复合噪声上的行为。

权重: models/noise_detector_efficientnetb0_rsna_{variant}_best.pth
输出: results/noise_detector_rsna/{variant}/

用法:
    python 10_train_noise_detector_rsna.py --variant 6class
    python 10_train_noise_detector_rsna.py --variant 7class
"""
import argparse
import sys
...
from rsna_commons import (DETECTOR_CLASSES, DETECTOR_DATASET_DIR, DETECTOR_VARIANTS,
...
def main():
    ...
    if not manifest.exists():
        raise FileNotFoundError(f"缺少 {manifest}，请先运行 09_build_noise_detector_dataset_rsna.py")
    ...
    plt.title(f"Noise Detector RSNA [{variant}] — val acc={best_acc:.4f}",
