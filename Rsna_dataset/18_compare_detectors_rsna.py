# -*- coding: utf-8 -*-
"""
RSNA 两个噪声判别器变体对比（与 CQ500 的 18 同构）
...
输入: results/optimality_rsna/{variant}/oracle_summary_rsna.csv
      results/noise_detector_rsna/{variant}/all_condition_behavior.csv
输出: results/detector_comparison_rsna/

用法: python 18_compare_detectors_rsna.py
"""
import sys
...
OUT = RESULT_ROOT / "detector_comparison_rsna"
...
def main():
    ...
    oracle, behavior = {}, {}
    for v in VARIANTS:
        o = load(RESULT_ROOT / "optimality_rsna" / v / "oracle_summary_rsna.csv")
        if o is not None:
            oracle[v] = o
        b = load(RESULT_ROOT / "noise_detector_rsna" / v / "all_condition_behavior.csv")
        if b is not None:
            behavior[v] = b

    if not oracle:
        raise SystemExit("没有找到任何变体的 oracle_summary_rsna.csv；"
    ...
    comp.to_csv(OUT / "detector_comparison_rsna.csv", index=False)
    ...
    if not delta_df.empty:
        delta_df.to_csv(OUT / "detector_variant_delta_rsna.csv", index=False)
    ...
    fig.suptitle("Detector variant comparison on compound unknown noise (RSNA ICH subset)",
                 fontweight="bold")
    fig.savefig(OUT / "fig_detector_comparison_rsna.png", dpi=200, bbox_inches="tight")
    plt.close()

    with open(OUT / "latex_detector_comparison_rsna.tex", "w", encoding="utf-8") as fh:
        fh.write(comp.to_latex(index=False, float_format="%.4f", escape=False,
                               caption="RSNA ICH subset: comparison of the 6-class and "
                                       "7-class noise detectors on compound unknown noise.",
                               label="tab:rsna_detector_variants"))
