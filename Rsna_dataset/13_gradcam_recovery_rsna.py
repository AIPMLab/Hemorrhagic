# -*- coding: utf-8 -*-
"""
RSNA 未知噪声恢复 Grad-CAM（与 CQ500 的 13 同构）

对 test 每位患者取一张中间切片，对比 Raw / Unknown Noisy / Adaptive Enhanced
三种输入的 Grad-CAM，用来展示"增强后注意力是否回到出血区域"。

需要 11 --save-images 产生的自适应增强图
    adaptive_unknown_tests/rsna/{variant}/{model}/...
输出: results/gradcam_recovery_rsna_{variant}_{model}/{patient}/*.png

用法:
    python 13_gradcam_recovery_rsna.py --variant 7class --model resnet50
"""
import argparse
import sys
...
from rsna_commons import (ADAPTIVE_ROOT, DATA_ROOT, MODEL_DIR, NOISY_ROOT, RESULT_ROOT,
...
OUT_DIR = RESULT_ROOT / f"gradcam_recovery_rsna_{args.variant}_{args.model}"
...
def main():
    ...
    if not ADAPTIVE_DIR.exists():
        raise SystemExit(
            ...
            f"请先运行 11_adaptive_unknown_pipeline_rsna.py --variant {args.variant} "
            f"--save-images")
