# -*- coding: utf-8 -*-
"""
19 TTA 消融（RSNA）：在复合未知噪声上，用测试时自适应把分类器的实际效果再抬一截
...
RSNA 侧的背景（与 CQ500 不同，解读时要留神）：
  - test 只有 77 名患者 / 2,772 张切片（CQ500 是 73 名 / 27,386 张）。
    患者级 1 名患者 = 1.3 个百分点，任何 <5% 的 Δ 都不该当结论 ——
    以 vs_none_p / vs_none_ci_* 的配对检验为准。
  - 本队列 Normal:Hemorrhagic = 46:31，多数类基线 0.5974；
    resnet50 在复合噪声下会塌成"全判 Normal"（召回 ~0.16），
    因此**务必同时看召回/AUC**，只看 accuracy 会把塌陷误读成"还行"。
    也正因如此，本数据集上先验校正（prior/priorq）预期收益比 CQ500 更明显。

用法:
    python 19_tta_unknown_rsna.py                      # 全部组合
    python 19_tta_unknown_rsna.py --tta none,bn,priorq # 只跑部分变体
    python 19_tta_unknown_rsna.py --limit-patients 5   # 冒烟
"""
import argparse
import sys
...
import rsna_commons as C  # noqa: E402  先导入它：它会确保 code 根在 sys.path 上
# 只从共享层取**数据集无关**的东西；路径与索引一律走 rsna_commons
...
RESULT_DIR = C.RESULT_ROOT / "tta_unknown_rsna"
NOISE = "unknown_mixed"

# key = 结果表里显示的短名（与 07/11/16 保持一致，便于 15/18 按 model 列对齐）
# value = (timm 架构名, 权重路径) —— 架构名必须完整，timm 不认 "swin_tiny" 这种简写
MODELS = {
    "resnet50": ("resnet50", C.MODEL_DIR / "resnet50_rsna_best.pth"),
    "swin_tiny": ("swin_tiny_patch4_window7_224",
                  C.MODEL_DIR / "swin_tiny_patch4_window7_224_rsna_best.pth"),
}
...
def load_filter_log(variant, model_name):
    ...
    p = C.RESULT_ROOT / "adaptive_unknown_rsna" / variant / \
...
def main():
    ...
    ap.add_argument("--target-prev", type=float, default=None,
                    help="先验校正的目标患病率；默认取 val 划分的患者级阳性率")
    ...
    print(f"[19] test 切片 {len(df)}，患者 {df['patient_id'].nunique()}，设备 {device}")

    # 先验校正的目标患病率：默认取 val 划分的患者级阳性率（标注参考集，与 test 无关）
    ...
    pd.DataFrame(norm_rows).to_csv(RESULT_DIR / "norm_layers_rsna.csv",
    ...
    out.to_csv(RESULT_DIR / "tta_unknown_results_rsna.csv",
               index=False, encoding="utf-8-sig")
    if infos:
        pd.DataFrame(infos).to_csv(RESULT_DIR / "tta_adaptation_info_rsna.csv",
                                   index=False, encoding="utf-8-sig")
    ...
    for _, r in base.iterrows():
        p = C.RESULT_ROOT / "adaptive_unknown_rsna" / r["detector_variant"] / \
    ...
    print(f"\n[19] 写出 {RESULT_DIR / 'tta_unknown_results_rsna.csv'}")
