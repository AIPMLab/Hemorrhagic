...
现在：
  - 全程内存态穷举（复用 rsna_commons.sweep_filters，即 CQ500 那份实现）。
  - **拟合 phase**：在 val 上对每个噪声条件穷举 10 个 filter，选出最优。
  - **评估 phase**：用 val 拟合出的 filter 在 test 上评估，并给出全网格表。

输出: results/enhanced_known_noise_rsna/
     best_filter_map_rsna.csv            <- 11 消费的 类别->filter 表
     best_filter_per_known_noise_rsna.csv
     all_enhanced_known_noise_rsna.csv   <- test 全网格
     val_grid_rsna.csv                   <- val 全网格（拟合依据）

用法:
    python 07_evaluate_enhanced_known_noise_rsna.py
    python 07_evaluate_enhanced_known_noise_rsna.py --val-limit 400 --noises gaussian
"""
import argparse
import sys
...
from rsna_commons import (DETECTOR_CLASSES, FILTER_NAMES, MODEL_DIR, NOISY_ROOT,
...
def main():
    ...
    val_df.to_csv(OUTDIR / "val_grid_rsna.csv", index=False)
    test_df.to_csv(OUTDIR / "all_enhanced_known_noise_rsna.csv", index=False)
    map_df.to_csv(OUTDIR / "best_filter_map_rsna.csv", index=False)
    map_df.to_csv(OUTDIR / "best_filter_per_known_noise_rsna.csv", index=False)
