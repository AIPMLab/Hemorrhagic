# -*- coding: utf-8 -*-
"""
20 边界位移校正（RSNA）：把先验校正铺到**全部噪声条件**（clean + 5 类已知噪声 + 复合）
...
动机（来自 CQ500 与 RSNA 的已有结果）
    ...
    即模型并没有丢失排序能力，而是整体把分数压到了阈值以下（本队列多数类基线 0.5974）。
...
产出
    ...
    以及由 AUC 推出的可恢复上限（binormal_ceiling），用于判断"还剩多少没拿到"。

RSNA 解读提醒
    test 只有 77 名患者：1 名 = 1.3 个百分点。Δ 小于 ~5% 时配对检验基本不可能显著，
    请以 vs_none_p / vs_none_ci_* 为准，不要只看 Δ 的正负。

用法:
    python 20_boundary_correction_rsna.py
    python 20_boundary_correction_rsna.py --conditions unknown_mixed
    python 20_boundary_correction_rsna.py --limit-patients 5   # 冒烟
"""
...
import rsna_commons as C  # noqa: E402
...
OUT_DIR = C.RESULT_ROOT / "boundary_correction_rsna"
...
def main():
    ...
    ap.add_argument("--target-prev", type=float, default=None,
                    help="覆盖目标患病率；默认取 val 划分的患者级阳性率")
    ...
    print(f"[20] 设备={device} 目标患病率={target:.4f}"
    ...
    defs = {"resnet50": ("resnet50", C.MODEL_DIR / "resnet50_rsna_best.pth"),
        ...
        for cond in conds:
            ...
            if args.dump_scores:
                ...
                for stag, sdf in (("plain", s_plain), ("bn", s_bn)):
                    ...
                    pp["bn"] = (stag == "bn")
                    pp["dataset"] = "RSNA"
                    pp.to_csv(sdir / f"per_patient_scores_rsna_{mname}_{cond}"
    ...
    path = OUT_DIR / "boundary_all_noise_rsna.csv"
