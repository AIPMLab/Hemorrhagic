...
与 CQ500 的 09 同构：
  * 源切片**只取 RSNA 的 train + val 患者，绝不使用 test**（否则判别器见过测试图）。
  * 每患者等间距抽 SLICES_PER_PATIENT 张。
  * 噪声一律来自 rsna_commons（= cq500_commons 同一份函数），
    与 04（分类器测试集加噪）同源，消除训练/推理噪声不匹配。
  * 7 个类别由**同一批源切片**变换而来，类别天然均衡。

manifest.csv 记录 file/class/patient_id/source_split，
使 10 能按患者划分判别器自身的 train/val（沿用 RSNA 的患者边界，两侧都不含 test）。

输出: noise_detector_dataset_rsna/{class}/{patient}_{idx}_{stem}.png + manifest.csv

用法:
    python 09_build_noise_detector_dataset_rsna.py
    python 09_build_noise_detector_dataset_rsna.py --slices-per-patient 40
"""
import argparse
import sys
...
from rsna_commons import (DATA_ROOT, DETECTOR_CLASSES, DETECTOR_DATASET_DIR,
