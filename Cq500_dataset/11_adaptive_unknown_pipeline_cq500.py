# -*- coding: utf-8 -*-
"""
CQ500 自适应未知噪声管线（per-slice），支持两个判别器变体

流程：
  1) 指定变体的判别器对 unknown_mixed 的**每张测试切片**预测噪声类别
       - 6class：只认识 6 类单一退化，对复合噪声是分布外，只能硬分到某个已知类
       - 7class：额外认识复合噪声，可直接判为 unknown_mixed
  2) 用 07 在 **val** 上拟合的 (类别 -> 最优 filter) 表，把类别换成 filter
  3) 对同一张切片施加该 filter，再送分类器
  4) 患者级投票，输出 raw / unknown_noisy / adaptive 三条件指标

输出（按变体分目录，两个变体结果并存便于对比）:
  results/adaptive_unknown_cq500/{variant}/adaptive_unknown_results_cq500.csv
  results/adaptive_unknown_cq500/{variant}/unknown_noise_detection_log_cq500.csv
  results/adaptive_unknown_cq500/{variant}/unknown_filter_selection_log_cq500.csv

用法:
    python 11_adaptive_unknown_pipeline_cq500.py --variant 6class
    python 11_adaptive_unknown_pipeline_cq500.py --variant 7class
"""
import argparse
import sys
...
from cq500_commons import (ADAPTIVE_ROOT, CLASSES, DATA_ROOT, DETECTOR_VARIANTS,
...
def variant_outdir(variant):
    return RESULT_ROOT / "adaptive_unknown_cq500" / variant
...
def load_filter_map():
    """读取 07 在 val 上拟合的 类别->filter 表。"""
    if not MAP_CSV.exists():
        raise FileNotFoundError(
            f"缺少 {MAP_CSV}，请先运行 07_evaluate_enhanced_known_noise_cq500.py")
...
def main():
    ...
    ap.add_argument("--variant", required=True, choices=sorted(DETECTOR_VARIANTS),
                    help="用哪个判别器变体：6class（仅单一噪声）或 7class（含复合噪声）")
    ap.add_argument("--limit", type=int, default=0, help="仅前 N 张切片（冒烟测试用）")
    ap.add_argument("--batch-size", type=int, default=32)
    ap.add_argument("--models", default=",".join(MODELS),
                    help="逗号分隔的模型列表，默认全部")
    ap.add_argument("--save-images", action="store_true",
                    help="另存自适应增强图到 adaptive_unknown_tests/cq500（13 出 Grad-CAM 需要）")
    args = ap.parse_args()
    ...
    # 分类器（各模型同时载入，共用 raw/noisy 的条件输入）
    classifiers = {}
        ...
        with torch.no_grad():
            for mname, model in classifiers.items():
                ...
                for rel, pid, cls_name, f in zip(chunk, pids[s:s + bs], classes, selected):
                    sel_rows.append({"model": mname, "patient_id": pid,
                                     "image_path": rel, "predicted_noise": cls_name,
                                     "selected_filter": f})

                x_ad = torch.stack([
                    tf(_pil(apply_filter(arr, f)))
    ...
    det_log.to_csv(OUTDIR / "unknown_noise_detection_log_cq500.csv", index=False)
    pd.DataFrame(sel_rows).to_csv(
        OUTDIR / "unknown_filter_selection_log_cq500.csv", index=False)

    summary = pd.DataFrame(rows).sort_values(["model", "condition"])
    summary.to_csv(OUTDIR / "adaptive_unknown_results_cq500.csv", index=False)
