...
TTA 变体：none / bn / tent / bn_tent（定义见 ich_common）
...
用法:
    python 19_tta_unknown_cq500.py                      # 全部组合
    python 19_tta_unknown_cq500.py --tta none,bn        # 只跑部分变体
    python 19_tta_unknown_cq500.py --limit 200          # 冒烟
"""
import argparse
import sys
...
import cq500_commons as C  # noqa: E402  先导入它：它会确保 code 根在 sys.path 上
# 只从共享层取**数据集无关**的东西；路径与索引一律走 cq500_commons
...
RESULT_DIR = C.RESULT_ROOT / "tta_unknown_cq500"
NOISE = "unknown_mixed"

# key = 结果表里显示的短名（与 07/11/16 保持一致，便于 15/18 按 model 列对齐）
# value = (timm 架构名, 权重路径) —— 架构名必须完整，timm 不认 "swin_tiny" 这种简写
MODELS = {
    "resnet50": ("resnet50", C.MODEL_DIR / "resnet50_cq500_best.pth"),
    "swin_tiny": ("swin_tiny_patch4_window7_224",
                  C.MODEL_DIR / "swin_tiny_patch4_window7_224_cq500_best.pth"),
}
...
def load_filter_log(variant, model_name):
    ...
    p = C.RESULT_ROOT / "adaptive_unknown_cq500" / variant / \
...
def run_combo(display, arch, weight, condition, tta, df, filter_by_path,
              device, args):
    """fresh 载入 -> 自适应 -> 评估，返回 (指标 dict, 自适应 info)。"""
...
def main():
    ...
    pd.DataFrame(norm_rows).to_csv(RESULT_DIR / "norm_layers_cq500.csv",
    ...
    out.to_csv(RESULT_DIR / "tta_unknown_results_cq500.csv",
               index=False, encoding="utf-8-sig")
    if infos:
        pd.DataFrame(infos).to_csv(RESULT_DIR / "tta_adaptation_info_cq500.csv",
                                   index=False, encoding="utf-8-sig")
    ...
    for _, r in base.iterrows():
        p = C.RESULT_ROOT / "adaptive_unknown_cq500" / r["detector_variant"] / \
    ...
    # ---- 关键对比：6class 上 TTA 带来的增量 ----
    print("\n[TTA 增益] adaptive 条件下，相对 tta=none 的 Δaccuracy")
    ...
    for (mn, v), g in ad.groupby(["model", "detector_variant"]):
        ...
        for _, r in g.sort_values("tta").iterrows():
            print(f"  {mn:9s} {v:8s} {r['tta']:8s} {r['accuracy']:.4f} "
                  f"({r['accuracy'] - b0:+.4f})")

    print(f"\n[19] 写出 {RESULT_DIR / 'tta_unknown_results_cq500.csv'}")
