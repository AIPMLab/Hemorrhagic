# -*- coding: utf-8 -*-
"""
20 边界位移校正：把先验校正铺到**全部噪声条件**（clean + 5 类已知噪声 + 复合）

动机（来自已有结果）
    噪声造成的端到端损失主要不是"排序坏了"，而是"决策边界位移"：
    resnet50 在 motion_blur 上 AUC 仍有 0.8233，准确率却只有 0.5479 —— 27.5 点落差。
    双正态模型估算：仅靠把边界挪回正确位置，motion_blur 可恢复到约 0.744（+19.6 点）。
...
产出
    results/boundary_correction_cq500/boundary_all_noise_cq500.csv
...
import cq500_commons as C  # noqa: E402
...
OUT_DIR = C.RESULT_ROOT / "boundary_correction_cq500"
...
def main():
    ...
    defs = {"resnet50": ("resnet50", C.MODEL_DIR / "resnet50_cq500_best.pth"),
        ...
        for cond in conds:
            ...
            if args.dump_scores:
                ...
                for stag, sdf in (("plain", s_plain), ("bn", s_bn)):
                    ...
                    pp["bn"] = (stag == "bn")
                    pp["dataset"] = "CQ500"
                    pp.to_csv(sdir / f"per_patient_scores_cq500_{mname}_{cond}"
    ...
    path = OUT_DIR / "boundary_all_noise_cq500.csv"
